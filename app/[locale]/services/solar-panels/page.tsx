...
export default function SolarPanelsPage() {
  const t = useTranslations('serviceDetails.solarPanels');
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map((num) => (
              <div key={num} className="bg-white p-8 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
                ...
                <h3 className="text-xl font-bold text-dark-700 mb-3">{t(`features.feature${num}.title`)}</h3>
                <p className="text-gray-600">{t(`features.feature${num}.description`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <h2 className="text-4xl font-bold text-dark-700 mb-12 text-center">{t('specs.title')}</h2>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-gradient-to-br from-primary-50 to-accent-50 p-8 rounded-2xl">
              <h3 className="text-2xl font-bold text-dark-700 mb-6">{t('specs.panel.title')}</h3>
              <ul className="space-y-4">
                {[1, 2, 3, 4, 5].map((num) => (
                  <li key={num} className="flex items-start">
                    <span className="text-primary-600 mr-3 text-xl">•</span>
                    <span className="text-gray-700">{t(`specs.panel.spec${num}`)}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="bg-gradient-to-br from-accent-50 to-primary-50 p-8 rounded-2xl">
              <h3 className="text-2xl font-bold text-dark-700 mb-6">{t('specs.inverter.title')}</h3>
              <ul className="space-y-4">
                {[1, 2, 3, 4, 5].map((num) => (
                  <li key={num} className="flex items-start">
                    <span className="text-accent-600 mr-3 text-xl">•</span>
                    <span className="text-gray-700">{t(`specs.inverter.spec${num}`)}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {[1, 2, 3, 4, 5].map((num) => (
              <div key={num} className="flex gap-6 items-start">
                ...
                <div className="flex-1 bg-white p-6 rounded-xl shadow-md">
                  <h3 className="text-xl font-bold text-dark-700 mb-2">{t(`process.step${num}.title`)}</h3>
